...
static bool _should_interact(Entity const &ent1, Entity const &ent2) {
    ...
    if ((ent1.has_component(kFlower) && ent1.get_color() == ColorID::kRed && ent2.has_component(kDrop)) ||
        (ent2.has_component(kFlower) && ent2.get_color() == ColorID::kRed && ent1.has_component(kDrop))) return false;
    ...
    if ((ent1.has_component(kFlower) && ent2.get_mob_id() == MobID::kTargetDummy) ||
        (ent2.has_component(kFlower) && ent1.get_mob_id() == MobID::kTargetDummy))return true;
    return false;
}
...
void on_collide(Simulation *sim, Entity &ent1, Entity &ent2) {
    ...
    if (dist < 0) return;
    if (NO(kDrop) && NO(kWeb) && NO(kPoisonWeb)) {
